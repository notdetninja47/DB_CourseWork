//
//  InsertionTypesTableViewController.swift
//  JewelryTrader
//
...
class InsertionTypeTableViewController: UITableViewController, NSFetchedResultsControllerDelegate {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = insertionType.name
        return cell
    }
    @IBAction func AddInsertionType(sender: UIBarButtonItem) {
        performSegueWithIdentifier("insertionTypesToInsertionType", sender: nil)
    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "insertionTypesToInsertionType" {
            let controller = segue.destinationViewController as! InsertionTypeViewController
            controller.entity = sender as? InsertionType
        }
    }
    
    override func tableView(tableView: UITableView, commitEditingStyle editingStyle: UITableViewCellEditingStyle, forRowAtIndexPath indexPath: NSIndexPath) {
        if editingStyle == .Delete {
            let entity = fetchedResultsController.objectAtIndexPath(indexPath) as! InsertionType
            if entity.insertions!.count == 0 {
